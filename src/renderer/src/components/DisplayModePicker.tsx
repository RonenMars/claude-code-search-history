...
const MODES: { value: DisplayMode; label: string; icon: JSX.Element }[] = [
  ...
  },
  {
    ...
    label: 'Tree',
    icon: (
      <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 640 640">
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        disabled={disabled}
        className="flex items-center gap-1.5 rounded border border-neutral-700 bg-neutral-800 px-1.5 py-1 text-[10px] text-neutral-400 transition-colors hover:text-neutral-200 disabled:pointer-events-none disabled:opacity-50"
        title={`Display: ${current.label}`}
      >
        {current.icon}
        <svg className={`h-2.5 w-2.5 text-neutral-500 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="absolute top-full right-0 z-50 mt-1 min-w-30 overflow-hidden rounded-lg border border-neutral-700 bg-neutral-800 shadow-xl">
          {MODES.map((mode) => (
            ...
            >
              ...
              {mode.value === value && (
                <svg className="text-claude-orange ml-auto h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
